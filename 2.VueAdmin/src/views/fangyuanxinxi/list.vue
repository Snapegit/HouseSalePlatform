	...
	<div>
		<div class="app-contain">
			...
			<el-table
				...
				<el-table-column label="房屋图片" width="120" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="审核" v-if="btnAuth('fangyuanxinxi','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('fangyuanxinxi','预约')" type="success" @click="yuyuegoufangCrossAddOrUpdateHandler(scope.row,'cross','是','[1]','已预约')">预约</el-button>
						<el-button v-if="btnAuth('fangyuanxinxi','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const approvalClick = (row) => {
		let params = {
			...
			meipingfangjia: row.meipingfangjia,
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussfangyuanxinxi?refid=' + id)
	}
    ...
    const yuyuegoufangCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			yuyuegoufangFormModelRef.value.init(row.id,'cross','预约',row,'fangyuanxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
