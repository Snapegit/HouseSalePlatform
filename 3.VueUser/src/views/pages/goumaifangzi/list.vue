	...
	<div class="app-contain" :style='{"padding":"0 18%","margin":"0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('goumaifangzi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="房屋类型" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.fangwuleixing}}
							</template>
						</el-table-column>
						<el-table-column label="房屋面积" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="销售价格" :resizable='true' align="left" header-align="left">
							<template #default="scope">
						...
						<el-table-column label="是否支付" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								...
								<el-button v-if="scope.row.ispay!='已支付' && btnAuth('goumaifangzi','支付')" type="text" size="small" @click.stop="payClick(scope.row)">支付</el-button>
							</template>
						</el-table-column>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/goumaifangziAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (searchQuery.value.xiaoshoujiagestart&&searchQuery.value.xiaoshoujiagestart!='') {
			params.xiaoshoujiagestart = searchQuery.value.xiaoshoujiagestart
		}
		if (searchQuery.value.xiaoshoujiageend&&searchQuery.value.xiaoshoujiageend!='') {
	...
	const tableDetailClick = (row) => {
		router.push('goumaifangziDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
